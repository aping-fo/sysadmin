...
<body class="sticky-header">
		...
		<div class="wrapper">
			<div class="row">
				<div class="col-sm-12">
					...
					代理ID:${subordinateID}旗下代理信息 &nbsp;&nbsp;&nbsp;
					<a  onclick = "openSearch()" title="搜索"  ><i class="fa fa-search"></i></a> &nbsp;&nbsp;&nbsp;
					<a onclick = "backingOut()" title="返回上一级" ><i class="fa fa-reply"></i></a>
	...
	<script type="text/javascript">
		...
		function openSearch(){
			layer.prompt({
				  title: '搜索代理(ID/昵称)'
					...
					location.href="${BASE_PATH}/agent/seeAgentInfoUI?searchContent="+value+"&subordinateID=${subordinateID}";
				  layer.close(index);
				});
		}
		// 返回上一级
		function backingOut(){
			if(${subordinateID}!=${agent.ID}){
				$.ajax({
			        url: "${BASE_PATH}/agent/seeSubordinateAgentID",
			        datatype: 'json',
			        type: 'post',
			        ...
			        success: function (data) {
			        	...
			        	if(data==${agent.ID}){
							...
							location.href="${BASE_PATH}/agent/seeAgentInfoUI?subordinateID="+data;
			        	}
			        }
				});				
			}
		...
		function page(curr) {
			layer.load(2);
			$.getJSON(
						'${BASE_PATH}/agent/getSubordinateAgentList',
						{
							pageIndex : curr || 1
						},
						function(res) {
