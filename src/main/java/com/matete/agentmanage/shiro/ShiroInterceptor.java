	...
	public void intercept(Invocation ai) {
		...
		if (ah != null) {
			try {
				 ...
				 * 
				 * 
				 * }else{ ai.getController().renderError(401); }
				 */
				if (ah instanceof GuestAuthzHandler) {
					ai.getController().redirect(ShiroKit.getSuccessUrl());
					return;
				}
				...
				System.out.println("權限url：" + ah.shiroUrl());
				ai.getController().redirect(ah.shiroUrl());
				/*
				 * if(StrKit.notBlank(ShiroKit.getUnauthorizedUrl())){
				 * ai.getController().redirect(ShiroKit.getUnauthorizedUrl());
				 * }else{ ai.getController().renderError(403); }
				 */
				return;
			}
		}
