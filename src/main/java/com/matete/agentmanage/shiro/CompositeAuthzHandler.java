...
class CompositeAuthzHandler implements AuthzHandler {
	...
	public String shiroUrl() {
		...
		for(AuthzHandler authzHandler : authzHandlers){
			if(authzHandler instanceof PermissionAuthzHandler){
				return ShiroKit.getPermissionAuthzUrl();
			}
			if(authzHandler instanceof GuestAuthzHandler){
				return ShiroKit.getGuestAuthzUrl();
			}
			if(authzHandler instanceof AuthenticatedAuthzHandler){
				return ShiroKit.getAuthenticatedAuthzUrl();
			}
			if(authzHandler instanceof RoleAuthzHandler){
				return ShiroKit.getRoleAuthzUrl();
			}
			if(authzHandler instanceof UserAuthzHandler){
